...
/**
 * @param {{id: string, value: number, unit?: string} | null} info - null hides the tooltip
...
export function showTooltip(info, screenPos) {
  ...
  valueEl().textContent = info.unit ? `${info.value.toFixed(1)} ${info.unit}` : `${info.value.toFixed(1)}`;
